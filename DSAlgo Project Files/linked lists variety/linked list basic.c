...
//            ptr=temp;
//            ptr->next=NULL;
//above two steps need not be done sincetemp->next always points to null in case of create function             }
    }
return;
}
...
{   printf("deletion in progress....\n");
    ptr=first;
    first=first->next;
    free(ptr);    //don't forget to free memeory
    return;
}
...
while(ptr!=NULL)
{
   next_ptr=ptr->next;  //since ptr->next no more points towards next node so storing in next_ptr , and used at last to go to the new node
   ptr->next=pre_ptr;   //reversing the pointing direction of ptr to null initially it was pointing 2nd node// and befor that storing the address of 2nd node so that we do not loose it
   pre_ptr=ptr;         //bringing pre_ptr to next node
   ptr=next_ptr;        //after one link is successful taking ptr to next node for (for reversing link again)
}
first=pre_ptr;          //printing should start from end so last node becomes first i.e. pre_ptr
}
...
void merging_ll()
{
    ...
    while(ptr1!=NULL && ptr2!=NULL)
    {
        ...
        if(ptr3==NULL)          //now when we have data to be inserted
        {                       //we will create temp for condition if no element present in new list
            ptr3=temp3;
            first3=temp3;
        }
        else                    //if elements present in list then just link it to new node created and
        {                       //bring ptr to latest node created
            ptr3->next=temp3;
            ptr3=temp3;
        }
    }
